...
class AddIncidentNoteCommandTest {

    private Alert sampleAlert() {
        MonitoredService service =
                new MonitoredService("Payment", "http://localhost:8081/health", MonitorType.MOCK);
        return new Alert(service, Severity.WARNING, "Payment is DEGRADED");
    }

    @Test
    void execute_recordsNoteActivity() {
        ...
        new AddIncidentNoteCommand(id, "meet", "Investigating the latency spike",
                alertRepository, activityRepository).execute();

        ArgumentCaptor<IncidentActivity> captor = ArgumentCaptor.forClass(IncidentActivity.class);
        verify(activityRepository).save(captor.capture());
        assertEquals(IncidentActivityType.NOTE, captor.getValue().getType());
        assertEquals("meet", captor.getValue().getAuthor());
        assertEquals("Investigating the latency spike", captor.getValue().getDetail());
    }

    @Test
    void execute_rejectsBlankNote() {
        AlertRepository alertRepository = mock(AlertRepository.class);
        IncidentActivityRepository activityRepository = mock(IncidentActivityRepository.class);

        AddIncidentNoteCommand command =
                new AddIncidentNoteCommand(UUID.randomUUID(), "meet", "   ", alertRepository, activityRepository);
    ...
    void execute_throwsWhenAlertMissing() {
        ...
        AddIncidentNoteCommand command =
                new AddIncidentNoteCommand(id, "meet", "note", alertRepository, activityRepository);
